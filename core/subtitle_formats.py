...
class SRTParser(SubtitleParser):
    ...
    # Regex patterns for cleaning ASS content when writing SRT
    _ASS_TAG_PATTERN = re.compile(r'\{[^}]*\}')
    _ASS_DRAWING_PATTERN = re.compile(r'\{[^}]*\\p[1-9][^}]*\}')

    @staticmethod
    def _clean_text_for_srt(text: str) -> str:
        """Strip ASS override tags and clean text for SRT output."""
        # Check for ASS drawing mode - these events are vector graphics, not text
        if SRTParser._ASS_DRAWING_PATTERN.search(text):
            return ''
        # Remove all ASS override tags {...}
        cleaned = SRTParser._ASS_TAG_PATTERN.sub('', text)
    ...
    def write(subtitle_file: SubtitleFile, output_path: Path) -> None:
        ...
        Strips ASS override tags, filters out drawing events, removes
        micro-duration animation frames, and deduplicates overlapping entries.
        ...
        """
        try:
            # First pass: clean, filter, and deduplicate events
            cleaned_events = []
            seen = set()  # (start_ms, end_ms, text) for deduplication
            for event in subtitle_file.events:
                # Clean ASS tags from text for SRT output
                clean_text = SRTParser._clean_text_for_srt(event.text)

                # Skip events that are empty after tag removal (drawings, effects-only)
                if not clean_text:
                    continue

                # Skip micro-duration animation frames (< 100ms)
                if (event.end - event.start) < 0.1:
                    continue

                # Deduplicate: skip if same timing and text already seen
                key = (round(event.start, 2), round(event.end, 2), clean_text)
                if key in seen:
                    continue
                seen.add(key)

                cleaned_events.append((event, clean_text))

            # Second pass: write deduplicated events
            with open(output_path, 'w', encoding='utf-8', newline='\n') as f:
                for idx, (event, clean_text) in enumerate(cleaned_events, start=1):
                    f.write(f"{idx}\n")
...
class ASSParser(SubtitleParser):
    ...
    @staticmethod
    def _detect_cjk_content(events: List[SubtitleEvent]) -> bool:
        """Check if any events contain CJK characters."""
        for event in events[:50]:  # Check first 50 events for performance
    ...
    def write(subtitle_file: SubtitleFile, output_path: Path) -> None:
        ...
        Automatically selects a CJK-compatible font when the content contains
        ...
        try:
            with open(output_path, 'w', encoding='utf-8-sig', newline='\n') as f:
                ...
                if subtitle_file.script_info:
                    ...
                    f.write('[Script Info]\n')
                    f.write('Title: Generated by Bilingual Subtitle Suite\n')
                ...
                if subtitle_file.styles:
                    ...
                    # Detect CJK content and choose appropriate font
                    has_cjk = ASSParser._detect_cjk_content(subtitle_file.events)
                    if has_cjk:
                        # Use Microsoft YaHei (standard Windows CJK font) with Arial fallback
                    ...
                    f.write(f'Style: Default,{font_name},{font_size},&H00FFFFFF,&H000000FF,&H00000000,&H00000000,-1,0,0,0,100,100,0,0,1,2.5,1,2,10,10,20,1\n\n')
